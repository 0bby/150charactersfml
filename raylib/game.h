...
#define UNIT_COLLISION_RADIUS                                                  \
  3.0f                       // circle-circle push radius for unit separation
#define BLUE_TEAM_MAX_SIZE 4 // player team cap (change this to rebalance)
#define ARENA_BOUNDARY_Z                                                       \
  5.0f // blue units can't be placed below this Z (into red territory)
...
typedef struct {
  ...
  int castAnimCount;
  int animIndex[ANIM_COUNT]; // index into respective anim array (-1 = not
                             // found)
  bool hasAnimations;
...
typedef enum {
  ...
  COMBAT_EVT_PROJECTILE_HIT, // unitIndex=target, value1=projType,
                             // position=impact pos
