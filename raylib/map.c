...
static const char *NODE_NAMES[] = {
    "Combat", "Elite", "Boss", "Shop", "Rest", "Event"
};
...
static NodeType PickNodeType(int layer, int prevLayerHasShop, int prevLayerHasRest) {
    // Layer 0: always combat
    if (layer == 0) return NODE_COMBAT;
    // Last layer: always boss
    if (layer == MAP_LAYERS - 1) return NODE_BOSS;
    ...
    for (int attempt = 0; attempt < 20; attempt++) {
        ...
        NodeType t;
        if (r < 0.45f)      t = NODE_COMBAT;
        else if (r < 0.57f)  t = NODE_ELITE;
        else if (r < 0.67f)  t = NODE_SHOP;
        else if (r < 0.82f)  t = NODE_REST;
        else                  t = NODE_EVENT;

        // Constraints
        if (t == NODE_ELITE && layer < 5) continue;
        if (t == NODE_SHOP && prevLayerHasShop) continue;
        if (t == NODE_REST && prevLayerHasRest) continue;
        return t;
    }
...
void GenerateMap(ActMap *map, int act, uint32_t seed) {
    ...
    for (int L = 0; L < MAP_LAYERS; L++) {
        int count;
        if (L == 0) count = 1;
        else if (L == MAP_LAYERS - 1) count = 1;
        else count = map_rng_range(2, MAP_MAX_NODES_PER_LAYER);
        ...
        if (L > 0) {
            for (int n = map->layerOffset[L-1]; n < map->layerOffset[L-1] + map->nodesPerLayer[L-1]; n++) {
                if (map->nodes[n].type == NODE_SHOP) prevShop = 1;
                if (map->nodes[n].type == NODE_REST) prevRest = 1;
            }
        }
    ...
    for (int L = 5; L <= 12 && L < MAP_LAYERS - 1; L++) {
        for (int n = map->layerOffset[L]; n < map->layerOffset[L] + map->nodesPerLayer[L]; n++) {
            if (map->nodes[n].type == NODE_ELITE) { hasElite = true; break; }
        }
        if (hasElite) break;
    }
    if (!hasElite) {
        // Force an elite on a random node in layers 5-12
        int targetLayer = map_rng_range(5, 12 < MAP_LAYERS - 2 ? 12 : MAP_LAYERS - 2);
        int ni = map->layerOffset[targetLayer] + map_rng_range(0, map->nodesPerLayer[targetLayer] - 1);
        map->nodes[ni].type = NODE_ELITE;
    }

    // Generate edges (paths between layers)
    // StS rule: paths must not cross. If node A.col < B.col, A's edges <= B's edges in column.
    for (int L = 0; L < MAP_LAYERS - 1; L++) {
        ...
        // Start with mandatory connections: map each current node to proportional next node
        ...
        for (int c = 0; c < curCount; c++) {
            int primary = (nextCount > 1) ? (c * (nextCount - 1) + (curCount - 1) / 2) / (curCount > 1 ? curCount - 1 : 1) : 0;
            if (primary >= nextCount) primary = nextCount - 1;
            minEdge[c] = primary;
            maxEdge[c] = primary;
        }
        ...
        for (int n = 0; n < nextCount; n++) {
            ...
            for (int c = 0; c < curCount; c++) {
                if (minEdge[c] <= n && maxEdge[c] >= n) { reachable = true; break; }
            }
            if (!reachable) {
                ...
                if (n < minEdge[bestC]) minEdge[bestC] = n;
                else maxEdge[bestC] = n;
            }
        }

        // Enforce no-crossing: minEdge and maxEdge must be non-decreasing
        for (int c = 1; c < curCount; c++) {
            if (minEdge[c] < minEdge[c-1]) minEdge[c] = minEdge[c-1];
            if (maxEdge[c] < maxEdge[c-1]) maxEdge[c] = maxEdge[c-1];
        }

        // Optionally add extra random edges (1-2 per node) within non-crossing bounds
        for (int c = 0; c < curCount; c++) {
            MapNode *node = &map->nodes[curBase + c];
            // Add all nodes in [minEdge, maxEdge] range
            for (int n = minEdge[c]; n <= maxEdge[c] && node->edgeCount < MAP_MAX_EDGES; n++) {
            ...
            if (map_rng_float() < 0.3f && node->edgeCount < MAP_MAX_EDGES) {
                ...
                if (extra < nextCount) {
                    ...
                    for (int c2 = c + 1; c2 < curCount; c2++) {
                        if (minEdge[c2] < extra) { ok = false; break; }
                    }
                    if (ok) node->edges[node->edgeCount++] = nextBase + extra;
                }
            }
        }
    }
...
int UpdateMap(ActMap *map) {
    ...
    if (wheel != 0) {
        ...
        if (mapScrollTarget < 0) mapScrollTarget = 0;
        float maxScroll = (MAP_LAYERS - 1) * 80.0f;
        if (mapScrollTarget > maxScroll) mapScrollTarget = maxScroll;
    }
    ...
    for (int i = 0; i < map->nodeCount; i++) {
        if (!map->nodes[i].available) continue;
...
void DrawMap(ActMap *map) {
    ...
    for (int L = 0; L < MAP_LAYERS; L++) {
        ...
        float layerY = offsetY + (float)(MAP_LAYERS - 1 - L) * layerSpacing; // bottom=layer 0, top=boss
    ...
    for (int i = 0; i < map->nodeCount; i++) {
        ...
        for (int e = 0; e < node->edgeCount; e++) {
            ...
            if (next < 0 || next >= map->nodeCount) continue;
            ...
            for (int t = 1; t <= 8; t++) {
                ...
                float u = 1.0f - f;
                float x = u*u*u*p0.x + 3*u*u*f*p1.x + 3*u*f*f*p2.x + f*f*f*p3.x;
                float y = u*u*u*p0.y + 3*u*u*f*p1.y + 3*u*f*f*p2.y + f*f*f*p3.y;
    ...
    for (int i = 0; i < map->nodeCount; i++) {
        ...
        const char *label;
        switch (node->type) {
            case NODE_COMBAT: label = "C"; break;
            case NODE_ELITE:  label = "E"; break;
            case NODE_BOSS:   label = "B"; break;
            case NODE_SHOP:   label = "$"; break;
            case NODE_REST:   label = "R"; break;
            case NODE_EVENT:  label = "?"; break;
            default:          label = "?"; break;
        }
    ...
    for (int L = 0; L < MAP_LAYERS; L++) {
        if (map->nodesPerLayer[L] == 0) continue;
        float ly = map->nodes[map->layerOffset[L]].y;
        if (ly < -20 || ly > sh + 20) continue;
    ...
    if (hoveredNode >= 0) {
        ...
        const char *tooltip;
        switch (node->type) {
            case NODE_COMBAT: tooltip = TextFormat("Combat (Floor %d)", node->layer + 1); break;
            case NODE_ELITE:  tooltip = TextFormat("Elite Fight (Floor %d)", node->layer + 1); break;
            case NODE_BOSS:   tooltip = TextFormat("ACT BOSS (Floor %d)", node->layer + 1); break;
            case NODE_SHOP:   tooltip = TextFormat("Shop - Buy abilities (Floor %d)", node->layer + 1); break;
            case NODE_REST:   tooltip = TextFormat("Rest - Heal 30%% HP (Floor %d)", node->layer + 1); break;
            case NODE_EVENT:  tooltip = TextFormat("? Event (Floor %d)", node->layer + 1); break;
            default:          tooltip = typeName; break;
        }
        int ttw = MeasureText(tooltip, 16);
        int ttx = (int)(mouse.x + 15);
        int tty = (int)(mouse.y - 10);
        if (ttx + ttw + 10 > sw) ttx = sw - ttw - 10;
