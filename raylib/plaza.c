...
{
    lobby->poolCount = 0;
    lobby->selectedSlot = -1;
    lobby->heroSelected = false;

    // First VALID_UNIT_TYPE_COUNT slots: one of each guaranteed
    for (int i = 0; i < VALID_UNIT_TYPE_COUNT && lobby->poolCount < LOBBY_POOL_SIZE; i++) {
        int type = VALID_UNIT_TYPES[i];
        if (SpawnUnit(units, unitCount, type, TEAM_RED)) {
            int idx = *unitCount - 1;
            ...
            // Roll rarity: 5% Legendary, 20% Rare, 75% Common
            ...
            PlazaUnitData *pd = &plazaData[idx];
            pd->zoneIndex = slot % 5;  // wrap for zone centers (roaming uses these)
            pd->isScared = false;
